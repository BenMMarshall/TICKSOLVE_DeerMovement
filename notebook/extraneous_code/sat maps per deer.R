...
deerLL <- deerLL %>%
  ...
  filter(!x == max(deerLL[deerLL$Animal_ID == "Roe13_F",]$x),
         !x == min(deerLL[deerLL$Animal_ID == "Roe09_M",]$x),
         !x == min(deerLL[deerLL$Animal_ID == "Roe01_F",]$x))

indiList <- vector("list", length = length(unique(deerLL$Animal_ID)))
names(indiList) <- unique(deerLL$Animal_ID)
for(id in unique(deerLL$Animal_ID)){
  # id <- unique(deerLL$Animal_ID)[1]
  ...
  zoom <- 14
  if(id %in% c("Roe14_M", "Fallow02_F")){
    zoom <- 13
  } else if(id %in% c("Fallow07_F")){
    zoom <- 12
  }

  map <- get_googlemap(c(centreLoc[[1]],
                         centreLoc[[2]]), zoom = zoom,
                       maptype = "satellite")
  ...
  satMapIndi <- ggplot() +
    ...
    theme(
      ...
      axis.title = element_text(size = 12, face = 3),
      axis.ticks = element_line(colour = "grey25"),
      axis.text = element_text(angle = 45, hjust = 1, vjust = 0.5)
    )

  indiList[[id]] <- satMapIndi

}

wrap_plots(indiList)

lapply(indiList, function(x){
  ggsave(plot = x, filename = here("notebook", "satMaps", paste0(x$labels$title, ".png")),
         width = 220, height = 220, dpi = 300, units = "mm")
  ggsave(plot = x, filename = here("notebook", "satMaps", paste0(x$labels$title, ".pdf")),
         width = 220, height = 220, dpi = 300, units = "mm")
})
...
aberOverview <- ggplot() +
  ...
  geom_point(data = deerLL  %>%
             ...
             pch = 21,
             colour = "white", size = 1.5) +
  geom_text(data = deerLL %>%
              filter(region == "Aberdeenshire") %>%
              group_by(Animal_ID) %>%
              summarise(xMean = mean(lon),
                        yMean = mean(lat)),
            aes(x = xMean, y = yMean, label = Animal_ID), size = 2.5, hjust = 0, vjust = 0.5) +
...
ggsave(plot = aberOverview, filename = here("notebook", "satMaps", "overviewAberdeenshire.png"),
       width = 220, height = 220, dpi = 300, units = "mm")
ggsave(plot = aberOverview, filename = here("notebook", "satMaps", "overviewAberdeenshire.pdf"),
       width = 220, height = 220, dpi = 300, units = "mm")

centreLoc_wess <- deerLL %>%
...
map_wess <- get_googlemap(c(centreLoc_wess[[1]],
                            centreLoc_wess[[2]]), zoom = 9,
                          maptype = "satellite")

backgroundImage_wess <- terra::rast(map_wess)

wessOverview <- ggplot() +
  geom_spatraster_rgb(data = backgroundImage_wess) +
  geom_segment(data = scaleLocationLL, aes(x = x - 0.001, y = y + 0.001,
                                           xend = xend - 0.001, yend = yend + 0.001),
               colour = "white", size = 1.5) +
  annotate("text", x = scaleLocationLL$x[1] - 0.001, y = scaleLocationLL$y[1] + 0.001,
           ...
           fontface = 2) +
  # geom_path(data = deerLL %>%
  #             filter(region == "Wessex"), aes(x = lon, y = lat, colour = month,
  #                                                    group = Animal_ID),
  #           linewidth = 0.8, alpha = 0.75) +
  geom_point(data = deerLL  %>%
               filter(region == "Wessex"), aes(x = lon, y = lat, fill = Sex, group = Animal_ID),
             pch = 21,
             colour = "white", size = 1.5) +
  geom_text(data = deerLL %>%
              filter(region == "Wessex") %>%
              ...
              summarise(xMean = mean(lon),
                        yMean = mean(lat)),
            aes(x = xMean, y = yMean, label = Animal_ID), size = 2.5, hjust = 0, vjust = 0.5) +
  coord_sf(xlim = c(centreLoc_wess[[3]] - 0.15, centreLoc_wess[[4]] + 0.15),
           ylim = c(centreLoc_wess[[5]] - 0.005, centreLoc_wess[[6]] + 0.005)) +
  scale_fill_viridis_d(option = "C") +
  scale_colour_viridis_d(option = "C") +
  labs(title = "Wessex Overview",
       x = "Longitude", y = "Latitude", colour = "Month", fill = "Month") +
  theme_bw() +
  theme(
    text = element_text(colour = "grey25"),
...
ggsave(plot = wessOverview, filename = here("notebook", "satMaps", "overviewWessex.png"),
       width = 220, height = 220, dpi = 300, units = "mm")
ggsave(plot = wessOverview, filename = here("notebook", "satMaps", "overviewWessex.pdf"),
       width = 220, height = 220, dpi = 300, units = "mm")
