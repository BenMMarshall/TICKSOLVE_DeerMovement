...
get_evaluations(biomodModels_fallow)

if(biomodModels_fallow@sp.name == "Dama.dama"){
  species <- "fallow"
} else {
  species <- "rodent"
}
...
evalModels <- bm_PlotEvalMean(bm.out = biomodModels_fallow)
...
ggsave(filename = here("figures", paste0(species, "_evalModels.png")),
       width = 180, height = 140, units = "mm")

varImportByModel <- bm_PlotVarImpBoxplot(bm.out = biomodModels_fallow, group.by = c('expl.var', 'algo', 'algo'))

varImportByModel$tab %>%
...
ggsave(filename = here("figures", paste0(species, "_varImportanceModels.png")),
       width = 240, height = 120, units = "mm")

targets::tar_source()
targets::tar_load("tar_biomodEns_fallow")
biomodEns_fallow <- tar_biomodEns_fallow
...
evalEns <- bm_PlotEvalMean(bm.out = biomodEns_fallow, group.by = 'full.name')
...
ggsave(filename = here("figures", paste0(species, "_evalEnsemble.png")),
       width = 180, height = 140, units = "mm")

varImport <- bm_PlotVarImpBoxplot(bm.out = biomodEns_fallow,
...
ggsave(filename = here("figures", paste0(species, "_varImportanceEnsemble.png")),
       width = 200, height = 120, units = "mm")

respCurve <- bm_PlotResponseCurves(bm.out = biomodEns_fallow,
                                   models.chosen = get_built_models(biomodEns_fallow)[c(5)],
                                   fixed.var = 'mean')
...
ggsave(filename = here("figures", paste0(species, "_responseDistanceEnsemble.png")),
       width = 200, height = 120, units = "mm")
...
ggsave(filename = here("figures", paste0(species, "_responseBinaryEnsemble.png")),
       width = 240, height = 120, units = "mm")
