...
keywords: roe deer, Capreolus capreolus
---
...
For the Roe Deer we made use of previously conducted analysis on their movements that aimed to determine their preference for various land use types, as well as how their movements are altered when within them <CITE>.
...
Instead we followed the workflow outlined by ___ <CITE>, using species distribution modelling to generate reasonable estimations of landscape resistance and then applying a circuit-theory approach <CITE - check wording> to determine how interconnected selected woodland patches were.

We collated occurrence data from GBIF and NBAtlass <CITE> for the Fallow Deer and Rodents, using searches from "Dama dama", "Myodes glareolus", "Apodemus sylvaticus", "Sciurus carolinensis", and "Sciurus vulgaris".
...
Once filtered we used CoordinateCleaner <CITE> to eliminate records that likely contained location errors.
...
We generated a number pseudo-absences data points for both target taxa equal to three times the number of true-occurrences available; we repeated this process ___ times allowing for multiple model fits.
...
We fitted a variety of models: Artificial Neural Network, Boosted Regression Trees, Generalized Linear Model, Maximum Entropy, and Random Forest. <!-- and eXtreme Gradient Boosting Training. --> <!-- XGBOOST can't be run with cat vars -->
We used block cross validation, balanced based on environmental covariates, to evaluate model performance <CITE>.
...
We included all models that obtained TSS scores of over 0.5, and weighted models proportionately to their TSS score.
Once the ensemble model had been created, we used the highest TSS mean ensemble model to project the probability of occurrence to the Aberdeenshire and Wessex landscapes. <!-- check exactly the wording, might be reprojecting all but only using one -->
...
In the case of the Fallow Deer we have sufficient data to parametrise the raod resistance based on the results of the Fallow Deer Poisson model; therefore, we alter the resistance of the cells that corresponded to the presence of a road in the same way as described for the Roe Deer.
...
For the Fallow Deer and Rodents, we followed ___ and used the Omniscape algorithm to generate a map of landscape connectivity <CITE>.
...
In both instances we based our search radius on previously recorded estimates of home range size, based heavily on the data provide by the home range database <CITE>.
