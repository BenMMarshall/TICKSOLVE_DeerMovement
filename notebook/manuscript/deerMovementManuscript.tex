...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{akdeSummary }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(here}\SpecialCharTok{::}\FunctionTok{here}\NormalTok{(}\StringTok{"tables"}\NormalTok{, }\StringTok{"allAKDESummary\_Aberdeen.csv"}\NormalTok{))}

\NormalTok{totalAberRange }\OtherTok{\textless{}{-}}\NormalTok{ akdeSummary }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(level }\SpecialCharTok{==} \FloatTok{0.95}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(}\FunctionTok{str\_detect}\NormalTok{(Animal\_ID, }\StringTok{"Roe"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pull}\NormalTok{(est) }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{range}\NormalTok{()}

\NormalTok{totalAberRange }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\FunctionTok{round}\NormalTok{(totalAberRange, }\AttributeTok{digits =} \DecValTok{1}\NormalTok{), }\AttributeTok{collapse =} \StringTok{" to "}\NormalTok{)}

\NormalTok{aberMean }\OtherTok{\textless{}{-}} \FunctionTok{readRDS}\NormalTok{(here}\SpecialCharTok{::}\FunctionTok{here}\NormalTok{(}\StringTok{"modelOutput"}\NormalTok{, }\StringTok{"aberdeenMeanAKDE.rds"}\NormalTok{))}

\NormalTok{ctmmMean }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\FunctionTok{round}\NormalTok{(aberMean}\SpecialCharTok{$}\NormalTok{CI[}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{], }\AttributeTok{digits =} \DecValTok{1}\NormalTok{),}
       \StringTok{" ha (95\% CI "}\NormalTok{, }\FunctionTok{round}\NormalTok{(aberMean}\SpecialCharTok{$}\NormalTok{CI[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{], }\AttributeTok{digits =} \DecValTok{1}\NormalTok{),}
       \StringTok{"{-}"}\NormalTok{, }\FunctionTok{round}\NormalTok{(aberMean}\SpecialCharTok{$}\NormalTok{CI[}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{], }\AttributeTok{digits =} \DecValTok{1}\NormalTok{),}
       \StringTok{")"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Home range sizes
\texttt{totalAberRange}
\texttt{ctmmMean}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{poisCoef }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(here}\SpecialCharTok{::}\FunctionTok{here}\NormalTok{(}\StringTok{"tables"}\NormalTok{, }\StringTok{"poisFixed.csv"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
For all analysis we used R (v.4.4.2) (\citeproc{ref-base}{R Core Team 2024}), and R Studio (v.2024.12.0+467) (\citeproc{ref-rstudio}{Posit team 2024}). For analysis of animal movement data we used amt (v.0.2.2.0) (\citeproc{ref-amt}{Signer, Fieberg, and Avgar 2019}), ctmm (v.1.2.0) (\citeproc{ref-ctmm}{Christen H. Fleming and Calabrese 2023}), and move (v.4.2.6) (\citeproc{ref-move}{Kranstauber, Smolla, and Scharf 2024}). For general data manipulation we used glue (v.1.8.0) (\citeproc{ref-glue}{Hester and Bryan 2024}), sjmisc (v.2.8.10) (\citeproc{ref-sjmisc}{Lüdecke 2018}), tidyverse (v.2.0.0) (\citeproc{ref-tidyverse}{Wickham et al. 2019}), and units (v.0.8.5) (\citeproc{ref-units}{E. Pebesma, Mailund, and Hiebert 2016}). For project and code management we used here (v.1.0.1) (\citeproc{ref-here}{Müller 2020}), tarchetypes (v.0.11.0) (\citeproc{ref-tarchetypes}{Landau 2021a}), and targets (v.1.9.0) (\citeproc{ref-targets}{Landau 2021b}). For visualisation we used the following as expansions from the tidyverse suite of packages: ggdist (v.3.3.2) (\citeproc{ref-ggdist2024a}{Kay 2024b}, \citeproc{ref-ggdist2024b}{2024a}), ggridges (v.0.5.6) (\citeproc{ref-ggridges}{Wilke 2024}), ggtext (v.0.1.2) (\citeproc{ref-ggtext}{Wilke and Wiernik 2022}), patchwork (v.1.3.0) (\citeproc{ref-patchwork}{Pedersen 2024}), and scales (v.1.3.0) (\citeproc{ref-scales}{Wickham, Pedersen, and Seidel 2023}). Other packages we used were boot (v.1.3.31) (\citeproc{ref-boot2024}{Angelo Canty and B. D. Ripley 2024}; \citeproc{ref-boot1997}{A. C. Davison and D. V. Hinkley 1997}), doParallel (v.1.0.17) (\citeproc{ref-doParallel}{Corporation and Weston 2022}), foreach (v.1.5.2) (\citeproc{ref-foreach}{Microsoft and Weston 2022}), and usethis (v.3.0.0) (\citeproc{ref-usethis}{Wickham et al. 2024}). To generate typeset outputs we used bookdown (v.0.42) (\citeproc{ref-bookdown2025}{Xie 2025}, \citeproc{ref-bookdown2016}{2016}), and rmarkdown (v.2.29) (\citeproc{ref-rmarkdown2024}{Allaire et al. 2024}; \citeproc{ref-rmarkdown2018}{Xie, Allaire, and Grolemund 2018}; \citeproc{ref-rmarkdown2020}{Xie, Dervieux, and Riederer 2020}). To manipulate and manage spatial data we used gdistance (v.1.6.4) (\citeproc{ref-gdistance}{van Etten 2017}), raster (v.3.6.30) (\citeproc{ref-raster}{Hijmans 2024a}), sf (v.1.0.19) (\citeproc{ref-sf2023}{E. Pebesma and Bivand 2023}; \citeproc{ref-sf2018}{E. Pebesma 2018}), sp (v.2.1.4) (\citeproc{ref-sp2005}{E. J. Pebesma and Bivand 2005}; \citeproc{ref-sp2013}{Bivand, Pebesma, and Gomez-Rubio 2013}), terra (v.1.7.83) (\citeproc{ref-terra}{Hijmans 2024b}), and tidyterra (v.0.6.1) (\citeproc{ref-R-tidyterra}{Hernangómez 2023}). To run models and explore model outputs we used effects (v.4.2.2) (\citeproc{ref-effects2019}{Fox and Weisberg 2019}, \citeproc{ref-effects2018}{2018}; \citeproc{ref-effects2003}{Fox 2003}; \citeproc{ref-effects2009}{Fox and Hong 2009}), INLA (v.24.6.27) (@ \citeproc{ref-INLA2013b}{Martins et al. 2013}; @ \citeproc{ref-INLA2015d}{Lindgren and Rue 2015}; \citeproc{ref-INLA2017e}{Rue et al. 2017}; \citeproc{ref-INLA2018f}{Bakka et al. 2018}; \citeproc{ref-INLA2016g}{De Coninck et al. 2016}; \citeproc{ref-INLA2017h}{Verbosio et al. 2017}; \citeproc{ref-INLA2018i}{Kourounis, Fuchs, and Schenk 2018}), lme4 (v.1.1.35.5) (\citeproc{ref-lme4}{Bates et al. 2015}), and performance (v.0.12.4) (\citeproc{ref-performance}{Lüdecke et al. 2021}).
...
A. C. Davison, and D. V. Hinkley. 1997. \emph{Bootstrap Methods and Their Applications}. Cambridge: Cambridge University Press. \href{https://doi:10.1017/CBO9780511802843}{doi:10.1017/CBO9780511802843}.
...
Angelo Canty, and B. D. Ripley. 2024. \emph{{boot}: Bootstrap r (s-Plus) Functions}.
...
Corporation, Microsoft, and Steve Weston. 2022. \emph{{doParallel}: Foreach Parallel Adaptor for the {``{parallel}''} Package}. \url{https://CRAN.R-project.org/package=doParallel}.
...
Hijmans, Robert J. 2024a. \emph{{raster}: Geographic Data Analysis and Modeling}. \url{https://CRAN.R-project.org/package=raster}.

\bibitem[\citeproctext]{ref-terra}
---------. 2024b. \emph{{terra}: Spatial Data Analysis}. \url{https://CRAN.R-project.org/package=terra}.
...
Kranstauber, Bart, Marco Smolla, and Anne K Scharf. 2024. \emph{{move}: Visualizing and Analyzing Animal Track Data}. \url{https://CRAN.R-project.org/package=move}.
...
---------. 2025. \emph{{bookdown}: Authoring Books and Technical Documents with r Markdown}. \url{https://github.com/rstudio/bookdown}.
