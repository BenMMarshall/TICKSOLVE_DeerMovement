...
\title{Tick Deer Connectivity Methods}
\author[1]{---*}
\author[2]{---**}
\author[3]{}
\author[4]{}
...
For the Roe Deer we made use of previously conducted analysis on their movements that aimed to determine their preference for various land use types, as well as how their movements are altered when within them .
...
Instead we followed the workflow outlined by Prima et al. (\citeproc{ref-prima_comprehensive_2024}{2024}), using species distribution modelling to generate reasonable estimations of landscape resistance and then applying a circuit-theory approach \textless CITE - check wording\textgreater{} to determine how interconnected selected woodland patches were.

We collated occurrence data from GBIF and NBN Atlas for the Fallow Deer and Rodents, using searches from ``\emph{Dama dama}'', ``\emph{Myodes glareolus}'', ``\emph{Apodemus flavicollis}'', ``\emph{Apodemus sylvaticus}'', ``\emph{Sciurus carolinensis}'', and ``\emph{Sciurus vulgaris}''.
...
Once filtered we used CoordinateCleaner to eliminate records that likely contained location errors on a species-by-species basis.
...
This process resulted in \_\_ occurrence locations of Fallow Deer and \_\_\_ for rodents (\_\_ \emph{Myodes glareolus}; \_\_ \emph{Apodemus flavicollis}; \_\_ \emph{Apodemus sylvaticus}; \_\_ \emph{Sciurus carolinensis}; \_\_ \emph{Sciurus vulgaris}).

We generated a number pseudo-absences data points for both target taxa equal to three times the number of true-occurrences available; we repeated this process \_\_\_ times allowing for multiple model fits .
...
For our bias layer, we used the human footprint index that combines a variety of human activities and land conversions into a single metric, and could serve as a proxy for the likelihood of observing our target species.

We used the biomod2 package to manage species distribution modelling .
...
We used stratified cross validation to evaluate model performance, as we were not priorisitisng model transferability \textless CITE \url{https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.12261}\textgreater.
We balanced stratification based on known occurrences both in x and y dimensions.
We elected to use True Skill Statistic (TSS) as our primary metric of model performance .
...
For the Fallow Deer and Rodents, we followed Prima et al. (\citeproc{ref-prima_comprehensive_2024}{2024}) and used Omniscape.jl (; built on Circuitscape.jl
To aid with computation costs, we selected a block size of \_\_ for the Fallow Deer, and \_\_ for the Rodents.
We also varied the search radius between the groups, as each has different movement/dispersal abilities.
In both instances we based our search radius on previously recorded estimates of home range size, based heavily on the data provide by the home range database .
In both case we filtered the home range data to species/group and calculated a mean home range area.
For the Fallow Deer their mean home range suggested a search radius of \_\textbf{; whereas for Rodents a search radius of } was chosen.
