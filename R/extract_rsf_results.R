#' Extract results and plot rsf outputs
#'
#' @name extract_rsf_results
#' @description Check models and plot outputs from individual RSF models.
#' @return Multiple plots and a table of model metrics.
#'
#' @export
extract_rsf_results <- function(deerData, rsfDataList, rsfModelList, nAvail){
  ...
  for(id in names(rsfModelList)){
    ...
    modelPerform <- performance::check_model(focalModel)

    ggsave(plot = plot(modelPerform),
           filename = here("modelOutput", paste0("rsfIndi_modelPerformance", id, ".png")),
           width = 240, height = 220, units = "mm", dpi = 300)

    newDataDistance <- data.frame(distancePatch = seq(0, 1200, 1),
                                  landuse = "Woodland")
    ...
    interactionPreds <- effect(term = "distancePatch*landuse",
                               mod = focalModel,
                               xlevels = list(distancePatch = seq(0, 1200, 20)),
                               KR = F)
